...
$(document).ready(function () {
    ...
    for (let k = 2; k < clue.length;) {
        for (let i = 0; i < clue[0]; ++i, k += clue[k] + 1) for (let j = 0; j < clue[k]; ++j) table_clue_row.find(`#clue_row_cell_${i}_${j + puzzle["max_clue_size"][0] - clue[k]}`).text(clue[j + k + 1]);
        for (let j = 0; j < clue[1]; ++j, k += clue[k] + 1) for (let i = 0; i < clue[k]; ++i) table_clue_col.find(`#clue_col_cell_${i + puzzle["max_clue_size"][1] - clue[k]}_${j}`).text(clue[i + k + 1]);
    }
    table_container.css({
        "display" : "grid"
    });
